...
- **2026-08-18**: **Agentic upgrade + Voyage embeddings** (laptop `hindsight-laptop` :8888, bank `rosebud`).
  ...
  - Follow-ups: (1) browser E2E under Voyage — blocked on detached ZCode in-app browser pane (reopen pane, then re-run retain/recall/full-loop UI probes with verbatim replies); (2) the nudge makes the model retry, but recall `budget`/re-ranker tuning (1500ms target, client ceiling 10s) is still open; (3) `dots_function_call` XML in the timeout-guard final pass is cosmetic.
