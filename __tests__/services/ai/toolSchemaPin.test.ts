...
const PINNED_OTHER_TOOLS_JSON = JSON.stringify([
    ...
    },
    {
        ...
        "function": {
            "name": "recall_memory",
            "description": "Query the long-term memory bank (Hindsight) as a FALLBACK when offline memory_search has nothing — prefer memory_search first (fully offline). Use for older echoes across past months. Do NOT use for recent days — use get_day or list_recent_days.",
            "parameters": {
                "type": "object",
                "properties": {
                    "query": {
                        "type": "string",
                        "description": "Topic or question to recall from long-term memory."
                    },
                    "limit": {
                        "type": "number",
                        "description": "Max recollections (1–10, default 6)."
                    }
                },
                "required": [
                    "query"
...
describe('PR8c tool schema pin (other than list_recent_days)', () => {
    it('fifteen non-list_recent_days tools are byte-identical to the pin', () => {
    ...
    it('registry still exposes exactly 16 history tools', () => {
