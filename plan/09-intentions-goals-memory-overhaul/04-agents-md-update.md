...
`AGENTS.md` is a behavioral correction layer: every rule must trace to a real failure mode. Phases A–C surfaced four new repo-specific failure modes that the current file does not prevent:

1. **`space-y-*`/`space-x-*` silently render zero spacing on native** (NativeWind v4) — caused the broken Goals UI. Now lint-able via the Phase A guard test.
2. **AsyncStorage read-modify-write races lose data** — the memory store was overwriting concurrent writes. Now there is a canonical lock pattern in `services/memory/localMemory.ts`.
3. **`JSON.parse` of storage payloads without try/catch crashes feature surfaces** — corrupted memory payload crashed journal finish, settings, and chat.
4. **Two parallel chat surfaces exist** — agents have forked chat UI per screen instead of reusing shared pieces (the old intention footer/input divergence).

The update also applies the meta-rules: every rule imperative and falsifiable, ordered by violation frequency, good/bad pairs over prose, and a storage-keys table (non-obvious, invisible from any single file). Rules that earn their place stay; nothing aspirational is added.

## Instruction

Replace the **entire contents** of `/home/sarino/Desktop/BlackroseJournal/AGENTS.md` with the content below, exactly. Do this as the last code change of the overall plan (after Phases A–C are merged), because rule 2 and the storage rules reference code those phases introduce.
...
- **WebView layers:** high-frequency rendering runs raw JS modules inside the WebView, not the RN bridge; state crosses via synchronized data bridges.
...
- `AGENTS.md` matches the block above exactly.
- The referenced guard test `__tests__/no-space-utilities.test.ts` exists (Phase A) and `services/memory/localMemory.ts` exposes `subscribeMemoryChanges` (Phase C) — both rules must point at real code.
- Update `PROGRESS.md`.
