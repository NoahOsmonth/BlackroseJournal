...
create table if not exists public.intention_checkins (
    id text primary key,
    owner_id uuid not null default auth.uid(),
    intention_id text references public.intentions (id) on delete set null,
    type text not null,
    title text,
    summary text,
    mood text,
...
create index if not exists intentions_owner_updated_idx
    on public.intentions (owner_id, updated_at desc);

create index if not exists intention_checkins_owner_updated_idx
    on public.intention_checkins (owner_id, updated_at desc);

create index if not exists goals_owner_updated_idx
    on public.goals (owner_id, updated_at desc);

create index if not exists personas_owner_updated_idx
    on public.personas (owner_id, updated_at desc);

create index if not exists saved_insights_owner_updated_idx
    on public.saved_insights (owner_id, updated_at desc);
...
create trigger intention_checkins_set_updated_at
before update on public.intention_checkins
for each row execute procedure public.set_updated_at();
...
create trigger saved_insights_set_updated_at
...
alter table public.intention_checkins enable row level security;
...
create policy "Intentions are private" on public.intentions
    for all using (owner_id = auth.uid()) with check (owner_id = auth.uid());

create policy "Intention checkins are private" on public.intention_checkins
    for all using (owner_id = auth.uid()) with check (owner_id = auth.uid());

create policy "Goals are private" on public.goals
    for all using (owner_id = auth.uid()) with check (owner_id = auth.uid());

create policy "Personas are private" on public.personas
    for all using (owner_id = auth.uid()) with check (owner_id = auth.uid());

create policy "Saved insights are private" on public.saved_insights
    for all using (owner_id = auth.uid()) with check (owner_id = auth.uid());
